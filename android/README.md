...
- A DataStore-backed settings screen with player selection, separate accent/settings-gradient colors with iOS preset swatches, subtitle/player defaults, default playback speed, configurable double-tap seek, two-finger play/pause, next-episode controls, classic/compact schedule layout controls, local/UTC schedule time, auto-mode, quality-threshold, similarity-algorithm, horror-filter, and auto-cache-clear controls, reader defaults, OAuth/manual tracker account controls, storage diagnostics, logger controls, and iOS-style catalog enable/reorder controls
