...
- Anime schedules through AniList, with AnimeSchedule fallback during outages
...
- [AnimeSchedule.net](https://animeschedule.net) provides the anime airing schedule fallback when AniList is unavailable. It supplies episode dates and delays while preserving schedule navigation and reminders. Configure `ANIMESCHEDULE_API_TOKEN` in the ignored `Build.local.xcconfig` for local builds or as a repository secret for CI builds. The AniList outage popup explains that schedules may be incomplete or out of date.
