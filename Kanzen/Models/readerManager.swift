...
class readerManager: ObservableObject {
    ...
    @AppStorage("readingMode", store: ProfileSettingsStore.active) var readingModeRaw: Int = ReadingMode.WEBTOON.rawValue
    var pagePrefetchers: [ReaderPinnedImagePrefetcher] = []
    var backgroundPagePrefetchers: [ReaderPinnedImagePrefetcher] = []
    ...
    private let readerReadThreshold: Double
...
var nextControllers: [UIViewController]?
    ...
    init(index: Int = 0, currChapter: [PageData] = [], prevChapter: [PageData] = [], nextChapter: [PageData] = [], shiftChapterLeft: @escaping () -> Void = {}, shiftChapterRight: @escaping () -> Void = {}, fetchPrev: @escaping () -> Void = {}, fetchNext: @escaping () -> Void = {}, kanzen: KanzenEngine,chapters: [Chapter]?, selectedChapter: Chapter?, mangaId: Int = 0, mangaTitle: String = "", mangaCoverURL: String = "", mangaRoute: MangaContentRoute? = nil, mangaFormat: String? = nil, totalChapters: Int? = nil, latestChapterNumbers: [String]? = nil, trackerAniListId: Int? = nil, trackerMALId: Int? = nil) {
        ...
        readerReadThreshold = (storedThreshold.isFinite ? min(max(storedThreshold, 50), 100) : 80) / 100
        self.index = index
        ...
        if mangaId != 0, let chapter = selectedChapter {
            ...
            if saved > 0, saved < currChapter.count {
                ...
                ReaderLogger.shared.log(
                    "Restoring reader page=\(saved + 1)/\(currChapter.count)",
                    type: "ReaderProgress"
                )
            }
    ...
    func generateCurrControllers()
    {
        currControllers = currChapter.map { page in
            if !page.isTransition, page.isImageLike {
                return UIHostingController(rootView: AnyView(ZoomablePageView(page: page).profileScopedAppStorage()))
            }
            return UIHostingController(rootView: AnyView(page.body.profileScopedAppStorage()))
        }
        if let selectedChapter = selectedChapter{
            ...
            currControllers = (currControllers ?? []) + [UIHostingController(rootView: AnyView(AnyView(transistionView).profileScopedAppStorage()))]
        }
    ...
    func generatePrevControllers()
    {
        ...
        if let selectedChapter = selectedChapter,
           ...
           selectedIndex > 0 {
            ...
            prevControllers = (prevControllers ?? []) + [UIHostingController(rootView: AnyView(AnyView(transistionView).profileScopedAppStorage()))]
    ...
    func generateNextControllers()
    {
        ...
        if let selectedChapter = selectedChapter,
           ...
           selectedIndex < chapters.count - 1 {
            let transistionView: any View = chapterView(page: PageData(content: "CHAPTER_END"), index: chapters[selectedIndex + 1].chapterNumber)
            nextControllers = (nextControllers ?? []) + [UIHostingController(rootView: AnyView(AnyView(transistionView).profileScopedAppStorage()))]
...
enum KanzenReaderMode: String, CaseIterable, Identifiable {
    ...
    static let builtInDefault = KanzenReaderMode.webtoon

    struct DefaultResolution: Equatable {
        let mode: KanzenReaderMode
        let recoveredStoredPreference: Bool
    }

    /// Pure resolution over an ordered store list, newest scope first. Reader
    /// mode moved from the device-level store to the profile store, so an
    /// existing preference has to be recoverable from either.
    static func resolveDefault(scopedKey: String, stores: [UserDefaults]) -> DefaultResolution {
        if scopedKey != "kanzenReaderMode" {
            for store in stores {
                if let scoped = store.object(forKey: scopedKey) as? String,
                   let mode = KanzenReaderMode(rawValue: scoped) {
                    return DefaultResolution(mode: mode, recoveredStoredPreference: false)
                }
            }
        }

        for store in stores {
            if let stored = store.object(forKey: "kanzenReaderMode") as? String,
               let mode = KanzenReaderMode(rawValue: stored) {
                return DefaultResolution(mode: mode, recoveredStoredPreference: true)
            }
            // `integer(forKey:)` answers 0 for an absent key, and 0 is LTR.
            // Only an explicitly stored number is a real preference; reading
            // the miss as a value forced paged mode on every fresh profile
            // and then persisted that guess.
            if let number = store.object(forKey: "readingMode") as? NSNumber,
               let legacy = ReadingMode(rawValue: number.intValue) {
                ...
                return DefaultResolution(mode: resolved, recoveredStoredPreference: true)
            }
        }

        return DefaultResolution(mode: builtInDefault, recoveredStoredPreference: false)
    }

    /// True when the stored global mode can only have been manufactured by the
    /// pre-fix default path. That path wrote `kanzenReaderMode` alone and, with
    /// `readingMode` absent, could only ever produce `.ltr`; a real selection
    /// always writes both keys together. Per-series overrides are user-written
    /// and are never inspected here.
    static func storedGlobalDefaultIsManufactured(in store: UserDefaults) -> Bool {
        store.object(forKey: "readingMode") == nil
            && (store.object(forKey: "kanzenReaderMode") as? String) == KanzenReaderMode.ltr.rawValue
    }

    static func currentDefault(scopeKey: String? = nil) -> KanzenReaderMode {
        let profileStore = ProfileSettingsStore.active
        let stores = profileStore == UserDefaults.standard
            ? [profileStore]
            : [profileStore, .standard]
        for store in stores where storedGlobalDefaultIsManufactured(in: store) {
            store.removeObject(forKey: "kanzenReaderMode")
        }
        let resolution = resolveDefault(
            scopedKey: storageKey(scopeKey: scopeKey),
            stores: stores
        )
        // Only a recovered preference is written back. Persisting an
        // unresolved default would freeze a guess into the profile.
        if resolution.recoveredStoredPreference,
           profileStore.object(forKey: "kanzenReaderMode") as? String != resolution.mode.rawValue {
            profileStore.set(resolution.mode.rawValue, forKey: "kanzenReaderMode")
        }
...
final class KanzenReaderSession {
    ...
    func loadSelectedChapter() async throws -> [KanzenReaderPage] {
        ...
        ReaderLogger.shared.log(
            "Reader loaded pages=\(mapped.count) mode=\(mode.rawValue)",
            type: "Reader"
        )
        return mapped
    }
...
struct KanzenReaderPageLoader {
    ...
    func loadPages(for chapter: Chapter, mode: KanzenReaderMode) async throws -> [PageData] {
        if let route,
           let localPages = ReaderDownloadManager.shared.pages(for: route, chapterNumber: chapter.chapterNumber) {
            ReaderLogger.shared.log("Reader using downloaded pages", type: "ReaderDownload")
            return localPages
        }
